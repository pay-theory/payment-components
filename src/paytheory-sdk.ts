...
/** Processor failure details attached to a failed transaction or tokenization. */
...
/**
 * Tokenization failure response. Returned by `tokenizePaymentMethod` when `expandedResponse` is
 * enabled and the processor rejects the payment method. Without `expandedResponse` the same
 * failure is reported as an {@link ErrorResponse}.
 */
...
export type TokenizeResult =
  ErrorResponse | TokenizedPaymentMethodResponse | FailedTokenizationResponse;
